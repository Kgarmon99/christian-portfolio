...
function deriveName(entry: SessionEntry): string {
...
function deriveChannel(entry: SessionEntry): string | undefined {
...
function deriveRuntime(entry: SessionEntry): string {
  return entry.agentRuntime?.id ?? 'auto'
}

function mapSession(entry: SessionEntry): Bot {
  const now = Date.now()
  ...
  return {
    ...
    contextTokens:
      typeof entry.contextTokens === 'number' ? entry.contextTokens : null,
  ...
  try {
    if (!hasOpenclaw()) {
      res.status(200).json({
        ...
        bots: [],
        warning:
          'openclaw CLI is not available in this serverless environment. Live bot data requires a host with openclaw installed.',
      })
      return
    }
    const raw = execSync(
      'openclaw sessions list --json --active 1440 --limit 200',
      {
        encoding: 'utf-8',
        timeout: 10_000,
    ...
    const sessions: SessionEntry[] = Array.isArray(parsed.sessions)
      ? parsed.sessions
      : []
